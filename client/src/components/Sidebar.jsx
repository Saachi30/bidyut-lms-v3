...
const Sidebar = ({ isOpen, toggleSidebar, onLogout, userRole: propUserRole }) => {
  const location = useLocation();
  const navigate = useNavigate();
  ...
  // For mobile, always show a small portion of the sidebar
  // even when "closed" to make it accessible
  const sidebarWidth = isOpen ? '16rem' : (isMobile ? '5rem' : '5rem');
  
  // No more transform that hides the sidebar completely
  const sidebarStyle = {
    width: sidebarWidth
  };
