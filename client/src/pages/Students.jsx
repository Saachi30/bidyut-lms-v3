...
const Students = ({ currUserRole }) => {
  ...
  // Handle enrollment for a student
  const handleEnrollStudent = async (studentId, grade) => {
    try {
      const response = await axios.post(`${baseURL}api/enrollments`, {
        studentId,
        grade,
        assignedById: '13', // Replace with current user ID
      });
      if (response.data.success) {
        alert('Student enrolled successfully!');
        // Refresh the students list
        const updatedStudents = students.map(student => 
          student.id === studentId ? { ...student, enrolled: true } : student
        );
